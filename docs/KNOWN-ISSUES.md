...
| PS-033 | A photograph uploaded and published through the admin does not display on the public site. A real upload's `media.storage_path` is a bucket-relative key generated server-side (e.g. `uploads/<uuid>.webp`) sitting in a **private** storage bucket with no public read path; the public query layer (`src/lib/queries/weddings.js`, `gallery.js`, `films.js`) returns that key verbatim, and the components that render it (`PhotoGallery`, `FeaturedStories`, `FilmsGallery`, `StoryDetailModal`) use it directly as an `<img src>` with no base-URL resolution, so it 404s. Only pre-existing seeded media renders today, because `scripts/seed-db.mjs` writes each seeded row's original full URL (an `images.unsplash.com` link or a local `/images/...` path) into `storage_path` directly, not a bucket key. `VITE_MEDIA_BASE_URL` exists and resolves a real URL, but only `src/admin/mediaUrl.js` reads it, for the admin's own media-library previews — nothing on the public site does. This matches the design doc's own scope: public read access is explicitly deferred to "a Cloudflare custom domain in front of R2," and Phase 3 never intended to wire the public site to it | Medium | `src/lib/queries/weddings.js`, `src/lib/queries/gallery.js`, `src/lib/queries/films.js`, `src/components/PhotoGallery.jsx`, `src/components/FeaturedStories.jsx`, `src/components/FilmsGallery.jsx`, `src/components/StoryDetailModal.jsx` | 4 |
