...
| `BookingForm` | Wedding-inquiry form section. Validates client-side with the shared `validateInquiry` rules, submits through `useInquirySubmission` (backed by the `submit-inquiry` Edge Function) with a Cloudflare Turnstile token from `useTurnstile`, and fires a `canvas-confetti` burst only once the inquiry is confirmed stored. | none | `formData` (object: `name`, `email`, `phone`, `weddingDate`, `venue`, `services[]`, `message`, `website` — the last is the honeypot), `clientErrors` (object, from client-side `validateInquiry`) | `ScrollReveal`; `canvas-confetti`; `WhatsAppButton` (rendered in the left contact column with the default message, and again in the failure panel with a message prefilled from whatever the couple had already typed — name, wedding date, venue, each guarded so a half-filled form doesn't leave dangling gaps); `useInquirySubmission`/`useTurnstile` (`src/hooks/`) drive `status` (`idle`\|`pending`\|`success`\|`error`), server-returned `fieldErrors`, and `retryAfterSeconds` (used to say roughly how long a `RATE_LIMITED` wait is, in minutes below an hour and "about an hour"/"about N hours" at or above it); `isInquiryBackendConfigured`/`TURNSTILE_SITE_KEY` (`src/lib/queries/inquiries.js`) gate whether the Turnstile widget mounts at all; contact details come from `src/data/contact.js`. The `<form>` carries `noValidate`, so `validateInquiry` — not the browser's own constraint validation on `type="email"` — is the single source of truth for what counts as a valid inquiry (native validation would otherwise block the submit event before `handleSubmit` runs, and would reject some legitimately-formatted addresses, e.g. non-Latin-script local parts, that `validateInquiry` accepts). While `status === 'pending'` the submit button is disabled and reads "Sending…"; on `status === 'error'` a panel renders copy keyed by the server's error code (always offering the studio email and, when configured, `WhatsAppButton`) plus any per-field `fieldErrors` inline. The `website` honeypot input is taken out of layout and given no visible size of its own — off-screen position plus zero opacity on the input itself, not just a clipping wrapper, and not `display:none`/`visibility:hidden` (which some spam bots skip, defeating the trap) — and is sent as an empty string unless something fills it; `aria-hidden` is set on both the input and its wrapper. Every visible input has `id`/`htmlFor` label pairing plus `aria-invalid`/`aria-describedby` wired to its inline error message. |
...
| `FeaturedStories` | Grid of wedding-story cards; clicking a card opens `StoryDetailModal` for that story. | `stories`, `onOpenLightbox`, `onOpenVideo` | `selectedStory` (null or a story object) | `ScrollReveal` (staggers each card by `index * 150ms`); `StoryDetailModal` (mounted conditionally once a card is selected); `data-cursor="EXPLORE ALBUM"`; tolerates an empty `stories` array (renders an empty grid, not a crash). |
| `FilmStrip` | Infinitely-scrolling "behind the lens" marquee of 6 analog-camera frame cards; the scroll is a CSS animation, not user-driven. | none | none | Reads `FILM_STRIP_FRAMES` (6 items) from `src/data/weddingData.js` (see "Components with hardcoded data (resolved)"); relies on the `animate-marquee` keyframe defined in `src/index.css`; `data-cursor="VIEW FRAME"`; renders `[...FILM_STRIP_FRAMES, ...FILM_STRIP_FRAMES]` (the list twice) to make the loop seamless. |
| `FilmsGallery` | 3-column grid of wedding-film thumbnails; clicking one calls `onOpenVideoModal` with that film's embed URL. | `films`, `onOpenVideoModal` | none | `ScrollReveal`; `data-cursor="PLAY FILM"`; `films` is passed in as a prop from `App.jsx` rather than imported directly, but like `FilmStrip`/`HorizontalGallery` it is ultimately sourced from `src/data/weddingData.js`; tolerates an empty `films` array (renders an empty grid, not a crash). |
| `Footer` | Site footer: brand blurb, social links, anchor-link navigation, and a back-to-top button. | none | none | `lucide-react` icons; `scrollToTop` calls `window.scrollTo({ top: 0, behavior: 'smooth' })`; rendered after `</main>` in `src/App.jsx`, outside the section stack. |
...
| `HorizontalGallery` | Horizontally-scrollable "editorial showcase" carousel with prev/next arrow buttons and a scroll-progress indicator bar. | none | `scrollProgress` (0–1, derived from the scroll container's `scrollLeft`/`scrollWidth`) | Reads `EDITORIAL_GALLERY` (5 items) from `src/data/weddingData.js` (see "Components with hardcoded data (resolved)"); `data-cursor="VIEW"`; injects a `<style>` tag to hide the scrollbar cross-browser. |
| `LightboxModal` | Fullscreen image viewer with prev/next navigation, `Escape`/arrow-key support, and a zoom toggle. | `activeImage`, `activeIndex`, `imagesList`, `onClose` | `currentIndex` (seeded from `activeIndex`), `isZoomed` | A `window` `keydown` listener for Escape/Arrow keys; both `useState` calls now run unconditionally before the `if (!activeImage) return null;` guard, so hook order is stable regardless of how the parent renders it (PS-006, resolved — see `docs/KNOWN-ISSUES.md`). |
| `Navbar` | Fixed top navigation bar with scroll-aware styling, anchor links, auth/role-aware action buttons, and a mobile hamburger drawer. | `onOpenBooking`, `user`, `onOpenAuthModal`, `onOpenClientGallery`, `onLogout` | `scrolled` (boolean), `mobileMenuOpen` (boolean) | A `window` `scroll` listener toggles `scrolled`; renders three different action-button layouts depending on whether `user` is absent, an admin, or a client — the admin variant is now a static "Admin Mode" badge (`<div>`, not a `<button>`) since it no longer opens anything on the public site; real content management lives in the separate admin app under `src/admin/`. |
| `PhotoGallery` | Filterable, layout-switchable (1/2/4 column) photo grid with per-category counts and a "zoom" lightbox trigger. | `photos`, `onOpenLightbox` | `activeCategory` ('All' by default), `layoutColumns` (1, 2, or 4) | `ScrollReveal` (header only); `data-cursor="ZOOM"`; `getGridClass()` switches the Tailwind grid classes based on `layoutColumns`; tolerates an empty `photos` array (renders an empty grid, not a crash). |
...
| `WhatsAppButton` | Renders a "Chat on WhatsApp" link to `wa.me` with a prefilled, URL-encoded message. Renders nothing at all when `VITE_WHATSAPP_NUMBER` is unset, so the site never ships a number the studio has not confirmed. | `message = DEFAULT_MESSAGE` (string), `className = ''` (string) | none | `WHATSAPP_NUMBER` (`src/data/contact.js`, sourced from `import.meta.env.VITE_WHATSAPP_NUMBER`); `lucide-react`'s `MessageCircle` icon; used by `BookingForm` in both the left contact column and the failure panel. |
...
- **Tab buttons.** `AuthModal` defines its own tab buttons using the
  ...
  flex items-center space-x-2 ..."` class string, varying only which tab is "active." This used
  to be duplicated a second time in `ContentManagerModal`, removed in Phase 3 (see "Components
  with hardcoded data (resolved)" below) — one copy remains, not zero, so the underlying
  duplication-to-consolidate item is unchanged, just smaller.
- **Pill/toggle buttons.** `ClientGalleryModal`'s all/favorites filter toggle and `PhotoGallery`'s
  ...
  (inactive) toggle logic. `Navbar`'s action buttons and `BookingForm`'s service-selection
  buttons follow the same pattern again with their own copies of the class string.
...
## Components with hardcoded data (resolved)

`FilmStrip` and `HorizontalGallery` used to define their own image arrays (`btsFrames` and
`galleryItems`, respectively) inline in the component file, instead of reading from
`src/data/weddingData.js` like every other data-driven section does. This was tracked as
`PS-015` in `docs/KNOWN-ISSUES.md`. Both arrays have since been moved into
`src/data/weddingData.js` as `FILM_STRIP_FRAMES` and `EDITORIAL_GALLERY` (see
`docs/DATA-MODEL.md`), and the components now import them from there. Note that this only makes
the data reachable by anything that reads `weddingData.js` (including Phase 1b's migration) — it
does not give either array a path into the database. `FilmStrip` and `HorizontalGallery` still
read `weddingData.js` directly rather than through a hook or the admin's real CRUD (`src/admin/`),
so content added through the admin cannot appear in the film-strip marquee or the horizontal
editorial carousel; this remains tracked as `PS-023` in `docs/KNOWN-ISSUES.md`. The Content
Manager modal that used to write ad hoc entries to `App.jsx` state and `localStorage` (neither of
which fed `weddingData.js` either) was deleted in Phase 3's admin cutover — see "Resolved" in
`docs/KNOWN-ISSUES.md` — so there is no longer any public-site path for adding photos or stories
at all; that capability now lives exclusively in the admin app under `src/admin/`.
