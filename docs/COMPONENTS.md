...
| --- | --- | --- | --- | --- |
| `AuthModal` | Sign-in modal, real since the client-portal rework: the Client Gallery tab checks the couple's access code against the `client_galleries_for_code` RPC (via `useClientAccess`) and signs in only a code that unlocks at least one published delivery — the code is handed up in `onLoginSuccess` and persisted with the session; the Studio tab is a plain link to the real admin app at `/admin.html` (the old fake email/password form checked nothing and is gone). | `isOpen`, `onClose`, `onLoginSuccess` | `activeTab` ('client'\|'admin'), `accessCode`, `errorMsg` | `useClientAccess` → `src/lib/queries/clientGalleries.js`; `lucide-react` icons. |
...
| `ClientGalleryModal` | The signed-in couple's delivery list: every published `client_galleries` entry matching their access code, each a title + description + "Open in Google Drive" link (`target="_blank"`). Fetches on every open via the stored code, with distinct loading / error-with-retry / being-prepared states. The old proofing grid over the public photo set (fake favourites, fake ZIP download) is deleted. | `isOpen`, `onClose`, `user` (whose `.code` drives the fetch) | none of its own — state lives in `useClientAccess` | `useClientAccess`; `lucide-react` icons. |
...
| `HomeVideo` | The Home page's cinematic hero band: the first published film plays **muted, looping, chromeless** as a full-width ambient background (natural 16:9), no text overlay — the clean video is the hero (Phase 3j removed the wordmark, mountain SVG, and tagline). When no film is published it shows a quiet dark band. Home-only; the Films/Stories pages keep their click-to-play modal with sound. | `film` (the first published film, or null) | none | `youtubeEmbedUrl(url, { background: true })` (`src/lib/youtube.js`) builds the ambient loop embed URL. |
...
| `Photo` | A photograph that tolerates an unresolvable source (`PS-036`). Renders a normal `<img>` when `src` is non-empty; when it is empty/null (what `publicMediaUrl()` returns for media with no public base URL configured) it renders a same-sized quiet block instead of the browser's broken-image glyph, so a grid keeps its shape. Used by `PhotoGallery`, `FeaturedStories`, `FilmsGallery`, and `StoryDetailModal`. | `src`, `alt = ''`, `className = ''`, plus any extra `<img>` props (e.g. `loading`) passed through | none | None beyond React. |
