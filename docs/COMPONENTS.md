...
| `Layout` | The frame every public page shares: `PetalsBackground` at `z-0`, `CustomCursor`, then a `z-10` wrapper holding `ScrollToTop`, `Navbar`, a `<main>` with the routed page via `<Outlet />`, and `Footer`. | `user`, `onOpenAuthModal`, `onOpenClientGallery`, `onLogout` (forwarded to `Navbar`); `contact` (forwarded to `Footer`) | none | `react-router-dom`'s `Outlet`; used as the layout route in `src/App.jsx`'s route table. |
