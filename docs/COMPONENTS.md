...
| `BookingForm` | Wedding-inquiry form section. Validates client-side with the shared `validateInquiry` rules, submits through `useInquirySubmission` (backed by the `submit-inquiry` Edge Function) with a Cloudflare Turnstile token from `useTurnstile`, and fires a `canvas-confetti` burst only once the inquiry is confirmed stored. | none | `formData` (object: `name`, `email`, `phone`, `weddingDate`, `venue`, `services[]`, `message`, `website` — the last is the honeypot), `clientErrors` (object, from client-side `validateInquiry`) | `ScrollReveal`; `canvas-confetti`; `WhatsAppButton` (rendered in the left contact column and again in the failure panel); `useInquirySubmission`/`useTurnstile` (`src/hooks/`) drive `status` (`idle`\|`pending`\|`success`\|`error`), server-returned `fieldErrors`, and `retryAfterSeconds`; `isInquiryBackendConfigured`/`TURNSTILE_SITE_KEY` (`src/lib/queries/inquiries.js`) gate whether the Turnstile widget mounts at all; contact details come from `src/data/contact.js`. While `status === 'pending'` the submit button is disabled and reads "Sending…"; on `status === 'error'` a panel renders copy keyed by the server's error code (always offering the studio email and, when configured, `WhatsAppButton`) plus any per-field `fieldErrors` inline. A visually hidden `website` honeypot input (`tabIndex={-1}`, `autoComplete="off"`, `aria-hidden`) is sent as an empty string unless a bot fills it. Every input has `id`/`htmlFor` label pairing plus `aria-invalid`/`aria-describedby` wired to its inline error message. |
