...
| `Layout` | The frame every public page shares: `ScrollToTop`, `Navbar`, a `<main>` holding the routed page via `<Outlet />`, and `Footer`. | `user`, `onOpenAuthModal`, `onOpenClientGallery`, `onLogout` (all forwarded to `Navbar`) | none | `react-router-dom`'s `Outlet`; used as the layout route in `src/App.jsx`'s route table. |
