...
the data reachable by anything that reads `weddingData.js` (including Phase 1b's migration) —
the "In-App Image Manager" (`ContentManagerModal`) still writes exclusively to `App.jsx` state
and `localStorage`, not to `weddingData.js`, so photos or stories added through it still cannot
appear in the film-strip marquee or the horizontal editorial carousel.
