...
| `npm run build` | `vite build && vite-node scripts/prerender.mjs` | Produces a production build in `dist/`, then prerenders it: `scripts/prerender.mjs` reads the published content through the real query modules and writes one flat HTML file per public route with its own `<head>` (title, description, canonical, Open Graph, JSON-LD) and a visible crawler shell, plus `sitemap.xml` and `build-info.json`. Without a reachable database it degrades to the six static routes (exit 0) everywhere except a Cloudflare Pages build, where it fails so the last deployment stays live — see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#build-pipeline-prerendered-heads-phase-5). `dist/` is gitignored (`PS-019`, resolved), so a build leaves the working tree clean. |
...
| `npm run prerender` | `vite-node scripts/prerender.mjs` | Reruns only the post-build prerender step against an existing `dist/` (needs `dist/index.html` from a prior `vite build`). Idempotent — it strips its own earlier output before writing. Reads `VITE_SITE_URL` for the canonical origin (falls back to `CF_PAGES_URL`, then `http://localhost:4173`) and `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` (or the plain `SUPABASE_*` names) for the database. |
| `npm run verify:prerender` | `vite-node scripts/verify-prerender.mjs` | Gate for the prerender: reads every published wedding and collection through `src/lib/queries/` and asserts `dist/` carries, for each, a flat `stories/<slug>.html` / `more/<slug>.html` with the right `<title>`, canonical, `og:image`, JSON-LD `ImageGallery` and shell `<h1>`; that the six static pages, `robots.txt` and `sitemap.xml` (listing every route) exist; that `build-info.json` is not degraded; and that no `index.html` sits under `dist/stories` or `dist/more`. Does not build — run `npm run build` first with the same `VITE_SITE_URL`/`VITE_MEDIA_BASE_URL`. Runs under `vite-node` for the same reason `verify:admin` does. Wired into CI's `admin-e2e` job. Same local-only guard as `db:seed`. |

Since Phase 5, `npm run build` is a two-step build — `vite build` and then the prerender — so the `dist/` it produces is already the crawler-visible site; `npm run preview` serves those flat files at their extension-less paths exactly as Cloudflare Pages does.
